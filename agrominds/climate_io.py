...
class ClimateBinReader:
    ...
    def __init__(
        ...
        landmap_path: Path, 
        climate_vars: list, 
        file_years: list = None,
        file_template: str = "gfdl-esm4_r1i1p1f1_w5e5_ssp126_{var_name}_global_daily_{year_from}_{year_to}.bin",
    ):
        ...
        if file_years is None:
            raise ValueError('Provide file years')
    ...
    def read(self, query: dict) -> dict:
        ...
        for var_idx, var_name in enumerate(self.climate_vars):
            for year_from, year_to, num_days in file_years:
                with open(os.path.join(self.data_dir, self.file_template.format(var_name=var_name, year_from=year_from, year_to=year_to)), mode='rb') as file:
